...
2. **Hardened Environment Configuration (Ansible)**:
   ...
   - **Multi-Layer Defense in Depth**:
     ...
     - **Observability Relays (loopback, read-only)**: For an agent that must see what its own stack reports, three declared prefixes on that same injection proxy — Prometheus, Alertmanager, Grafana — with the verb as the closure (`GET` only, so a silence or an annotation is refused by the method) and three reads closed by name, the Grafana datasource proxy among them. Two of the three have no authentication of their own, so their bound is the path list itself; Grafana's is its service account's role.
...
- `observability_relay_enabled`: Deploy the **read-only** relays to the agent's own observability stack (default: `false`). Requires `token_proxy_enabled`, and nothing is installed on the endpoints. The three upstreams are independent: declare the ones you want, and each becomes a set of loopback locations — see step 6 below.
- `observability_relay_{prometheus,alertmanager,grafana}_upstream`: bare origins for the three services, reached by name (`https://prometheus.example.com`). Reads are admitted on `/api/v1/`, `/api/v2/` and `/api/`, exactly and with their subtree, `GET` only — so an Alertmanager silence or a Grafana annotation is refused by the verb, not by a rule to maintain. **Prometheus and Alertmanager have no authentication of their own**, so the path list is their only bound; three reads are closed by name all the same: Grafana's `/api/datasources/proxy`, a tunnel to every configured datasource, and the paths that expose or rewrite a loaded configuration.
- `observability_relay_grafana_token_src`: **Required for the Grafana relay** — path on the control node to a service-account token, one line (a secret, so a file). The account's **Viewer** role is what bounds a read, exactly as the ClusterRole does for the Kubernetes broker.
- `token_proxy_enabled`: Deploy the token injection proxy, the primitive a broker declares its endpoints into (default: `false`). Required by `git_broker`'s HTTP half: loopback only, under the trust-zone user, and the agent never holds the credential it injects. Every upstream must be `https://` — a plain one would put the injected credential on the wire and turn the certificate check into a no-op — and a **bare origin**: as soon as it carries a path, even a lone trailing slash, nginx rewrites the forwarded path, which hands the agent every path of that origin under the injected credential. The paths belong in the locations, not in the upstream. An entry may also carry `refused_paths` — regexes refused with a 403 at the boundary, which is how the git broker closes the merge route. Those regexes become `location` blocks of the **whole server**, not of that entry: a path refused by one entry is refused for every entry, so relocating a prefix into another service's tree can refuse paths there too — fail-closed, but worth knowing. An entry may carry **no credential at all** — `auth_header` absent or empty — for a service whose API is anonymous: what the entry is worth is then the boundary, and the agent's own `Authorization` is dropped rather than injected. Its access trail goes to the journal (`journalctl -u token-proxy`), where journald bounds it: the agent drives that volume, so it must not be a file it can grow.
- `token_proxy_port`: Loopback port the token proxy listens on (default: `8002`).
- `token_proxy_ca_bundle`: the trusted bundle nginx verifies the upstream against when an entry declares no `ca_src` (default: the system's `ca-certificates`). Verification also needs a chain depth: nginx defaults to `1`, and a Let's Encrypt chain needs **2** — the certificate above the intermediate is resolved from the bundle, not sent — so `token_proxy_ssl_verify_depth` (default: `5`) is set on every block. Without it the handshake fails with `unable to get local issuer certificate`, which reads like a trust-store problem and is not one.
...
### 6. Observability Relays (optional)

Off by default. They answer what neither broker can: **reading the state of what the agent operates** — what is firing right now, which targets are down, which dashboard describes this service. Those services usually live *in* the cluster the Kubernetes broker reads, but they are reached by name, which the agent zone cannot do: its only way out is the loopback.

So a relay is the token proxy's own mechanism, declared once per service: no new component, nothing installed on the endpoints, and no credential in the agent zone. **Two of the three have no authentication at all** — a property of the software, not a gap here: Prometheus and Alertmanager ship an anonymous API, and anything in front of them (a Basic at the ingress, an authenticating proxy, their own `web.config.file`) is something a deployment added. Grafana authenticates by itself, so it is the one the relay carries a credential for.

**On the endpoints**:

- **Grafana** — a **service account** with the **Viewer** role, restricted to the folders the agent should see. That role is the bound that counts, exactly as the ClusterRole is for the Kubernetes broker: the relay admits reads on `/api/`, and the account decides what a read returns.
- **Prometheus and Alertmanager** — nothing to arrange if they answer reads anonymously. If they do not, their entry declares the header the front expects (`auth_header`, plus `basic_user` for a `Basic`), and that credential then lives in the trust zone like Grafana's.

**And on this machine**: it must **resolve the three names** and reach their port `443` — a public resolver knows nothing of an internal domain, so the deployment's DNS is declared at provisioning time (`dns_servers`, `iac/`). The play checks all three before it touches anything, and the proxy resolves its upstreams when it loads its configuration.

**Then produce the Grafana token** the play consumes:

```bash
printf '%s\n' '<the service account token>' > ansible/files/grafana.token
chmod 600 ansible/files/grafana.token
```

**From the agent zone**, the three APIs are read-only:

```bash
curl -s 'http://127.0.0.1:8002/api/v1/query?query=up%20%3D%3D%200'   # Prometheus: what is down
curl -s 'http://127.0.0.1:8002/api/v2/alerts?active=true'            # Alertmanager: what is firing
curl -s 'http://127.0.0.1:8002/api/search?type=dash-db'              # Grafana: the dashboards
```

Each declared prefix is admitted **exactly** and with its subtree, as for the git broker, and the verb is the closure: `GET` only, so no write route has to be closed by its name. Three reads are closed by name all the same, because reading them is the dangerous part: Grafana's `/api/datasources/proxy`, which is a tunnel to every configured datasource, and the two paths that expose or rewrite a loaded configuration (`/api/v1/status/config` and `/api/v1/admin` in Prometheus, `/api/admin` in Grafana).

What the agent can then see — targets, labels, metrics, alerts, dashboards — is broader than before, and labels carry hostnames, service names and image tags. Nothing there is a credential; and for the two anonymous endpoints there is no far-side bound at all, so the path list above **is** their bound. That asymmetry is deliberate and worth knowing.

A container reaches these addresses only by sharing the host's network namespace, as for the other brokers.
...
- **Observability Relays (loopback)**: The agent reads what its own stack reports — what is firing, which targets are down, which dashboard describes a service — through three declared prefixes on the same proxy, with no credential of its own and no component installed on the endpoints. The verb is the closure: `GET` only, so a silence in Alertmanager or an annotation in Grafana is refused by the method rather than by a rule someone maintains, and the Grafana datasource proxy — a tunnel to every configured datasource — is closed by name, as are the paths exposing or rewriting a loaded configuration. For Prometheus and Alertmanager, which ship an anonymous API, that path list *is* the bound: there is no far-side permission to lean on, and the honest reading is that the relay narrows what the agent can reach, not what those services would answer an anonymous client. Grafana's bound is its service account's **Viewer** role, held in the trust zone like every other credential here.
